package com.rockthejvm.interview.recap
...
object ScalaRecap extends App {
  ...
  def quickSort[T](list: List[T])(implicit ordering: Ordering[T]): List[T] = {
    ...
    if (list.size <= 1) list
      ...
      val pivot = list.head
      val smaller = list.tail.filter(ordering.lt(_, pivot))
      val greater = list.tail.filter(ordering.gt(_, pivot))
      quickSort(smaller) ++ List(pivot) ++ quickSort(greater)
    }
  }

  println(quickSort(List(7,8,9,10,1,2,3,4,5,6)))
}
